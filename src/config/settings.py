"""Application configuration for AWS Architecture Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="aws-architecture-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/aws_architecture_agent",
        description="Database connection URL",
    )
    redis_url: str = Field(default="redis://localhost:6379/0", description="Redis URL")

    llm_model: str = Field(default="anthropic.claude-3-5-sonnet-20241022-v2:0", description="LLM model ID")
    embedding_model: str = Field(default="amazon.titan-embed-text-v2:0", description="Embedding model")
    ...
    opensearch_index: str = Field(default="aws_architecture_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="aws-architecture-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="aws-architecture-agent", description="A2A agent ID")
